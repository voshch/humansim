...
class InferenceClient(ABC):
    @abstractmethod
    def generate(self, contents: str | list[str], system_instruction: str | list[str]) -> str | None:
...
class GoogleGenAIClient(InferenceClient):
    ...
    def generate(self, contents: str | list[str], system_instruction: str | list[str]) -> str | None:
        ...
        res = self.inference_client.models.generate_content(model=self.model, contents=contents, config=GenerateContentConfig(system_instruction=system_instruction, thinking_config="low"))
        answer = res.text
